...
namespace Freshdesk
{
    public class PagingRestClient : RestClient
    {
        ...
        public (Response, T) ExecuteFiltered<T>(IRestRequest request) where T : class, new()
        {
            ...
            FilteredResults<T> initialResults = JsonConvert.DeserializeObject<FilteredResults<T>>(response.Content, jsonSerializerSettings);
            ...
            for (int page = 2; page <= pages; page++)
            {
                ...
                FilteredResults<T> additionnalResults = JsonConvert.DeserializeObject<FilteredResults<T>>(response.Content, jsonSerializerSettings);
        ...
        public async Task<(Response, T)> ExecuteFilteredTaskAsync<T>(IRestRequest request, CancellationToken cancellationToken = default) where T : class, new()
        {
            request.Resource = AppendParametertoUri(request.Resource, "page", "1");
